			...
			function initBind($http, $scope,$timeout) {
				...
				$('#table_id_example').on('click', '.delete', function () {
					...
					swal({
	   	            	...
	   	            	$.ajax({
	   	 				url:'/admin/order/deleteAdminOrder',
	   	 				data:{
	   	 			    	"oId":params
	   	 			    },
	   	 				type:'post',
				...
				$('#table_id_example').on('click', '.update', function () {
					var params=$(this).attr('data-id');
					$.ajax({
	   	 				url:'/admin/order/updateSelectAdminOrder',
	   	 				data:{
	   	 			    	"rId":params
	   	 			    },
	   	 				type:'post',
				...
				$("#updateReceptinfo").on('click', function () {
					...
					$http({
   			            method: 'POST',
   			            url:'/admin/order/updateAdminOrder',
   			            headers: {
				...
				$('#table_id_example').on('click', '.pushSnack', function () {
					...
					swal({
	   	            	...
	   	            	$.ajax({
		   	 				url:'/admin/order/userOrderConfirm',
		   	 				data:{
		   	 			    	"id":params,
		   	 			    	"typeNum":0
		   	 			    },
				...
				$('#table_snack_example').on('click', '.updateOrderDetail', function () {
					var params=$(this).attr('data-id');
					$.ajax({
	   	 				url:'/admin/order/updateSelectAdminOrderDetail',
	   	 				data:{
	   	 			    	"dId":params
	   	 			    },
	   	 				type:'post',
	   	 				...
	   	 				success: function (result) {
	   			        	...
	   			        	$("#picUrl").attr('src',"/image/"+result.data.picUrl);
				...
				snackDatatable=$('#table_snack_example').DataTable({
					    ...
					    columns:[
							{ data:'picUrl',render: function ( data, type, row ) {
							    return "<img height='60' width='60' src='/image/"+data+"'/>";
							}},
					    	{ data:'sName'},
					    	{ data:'oMoney'},
					    	{ data:'oNum'},
			...
			function createTable(){
				if(datatable!=null){
					...
					datatable=$('#table_id_example').DataTable({
					    ...
					    serverSide: true,
					    ajax:{
					    url:"/admin/order/adminOrderLimit",
					    dataSrc:"data",
					    data: {
					    	"oId":$("#oId").val(),
