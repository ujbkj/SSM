	var integralModule = angular.module("adminSnackinfoApp",[]);
			...
			function initBind($http, $scope) {
				...
				$('#addAdminSnackinfo').on('click', function () {
		            ...
		            var data = formFile;
		            $.ajax({
	   	 				url:'/admin/snackinfo/addAdminSnack',
	   	 				data:data,
	   	 				type:'post',
	   	 				dataType:"json",
				...
				$('#table_id_example').on('click', '.update', function () {
					...
					$.ajax({
	   	 				url:'/admin/snackinfo/selectAdminUserinfoBysId',
	   	 				data:{
	   	 			    	"sId":params
	   	 			    },
	   	 				type:'post',
							...
							//alert(result.errCode)
	   			        	if(result.errCode='000000'){
	   			        		...
	   			        		$("#picUrlU").attr('src',"/image/"+result.data.sPictureurl);
				...
				$('#table_id_example').on('click', '.delete', function () {
					...
					swal({
	   	            	...
	   	            	$.ajax({
	   	 				url:'/admin/snackinfo/delAdminUserinfo',
	   	 				data:{
	   	 			    	"sId":params
	   	 			    },
	   	 				type:'post',
			...
			function createTable(){
				if(datatable!=null){
					...
					datatable=$('#table_id_example').DataTable({
					    ...
					    serverSide: true,
					    ajax:{
					    url:"/admin/snackinfo/adminSnackinfoLimit",
					    dataSrc:"data",
					    data: {
					    	"sType":$("#sType").val(),
					    	"sName":$("#sName").val(),
